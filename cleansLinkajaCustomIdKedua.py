...
def process_digipos_v2(df, location):
    ...
    df_filtered['custom_transaction_id'] = df_filtered.apply(
        lambda row: f"{row['Opposite Party'].split(' - ')[0].strip()}_{parse_datetime(row['Completion Time']).strftime('%d/%m/%Y')}_{parse_datetime(row['Completion Time']).strftime('%H.%M')}", axis=1)
...
def main():
    ...
    # Upload file Digipos
    uploaded_file_digipos = st.file_uploader("Upload File Digipos", type=['csv', 'xlsx'], key="file_uploader_digipos_v2")
    ...
    if st.button("Lakukan Cleansing dan Custom ID (v2)"):
        ...
        if 'df_digipos_v2' in st.session_state:
            try:
                cleaned_digipos = process_digipos_v2(st.session_state.df_digipos_v2, loc_prefix)
    ...
    if cleaned_digipos is not None:
        ...
        st.download_button(
            label="Download Hasil Digipos (v2)",
            data=excel_digipos,
